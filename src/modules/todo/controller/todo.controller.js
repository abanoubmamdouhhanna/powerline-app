...
export const addTaskDocument = asyncHandler(async (req, res, next) => {
  ...
  // Prepare folder for Cloudinary uploads
  ...
  // Add document to task and save
  task.documents.push(newDocument);
  await task.save();

  // Get the requested language or default to English
  const targetLanguage = req.language || "en";

  // Convert the task to a plain object and transform multi-language fields
  const taskObj = task.toObject();
  
  // Transform multi-language fields to return only the requested language
  const transformedTask = {
    ...taskObj,
    taskName: taskObj.taskName[targetLanguage] || taskObj.taskName.en,
    taskDetails: taskObj.taskDetails[targetLanguage] || taskObj.taskDetails.en,
    comment: taskObj.comment[targetLanguage] || taskObj.comment.en,
    documents: taskObj.documents.map(doc => ({
      ...doc,
      title: doc.title[targetLanguage] || doc.title.en,
      // Keep other document fields as is
      files: doc.files,
      _id: doc._id,
      start: doc.start,
      end: doc.end
